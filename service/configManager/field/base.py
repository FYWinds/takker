...
LastEditTime : 2022-11-30 11:33:06
FilePath     : /service/configManager/field/base.py
...
class ConfigKey:
    ...
    def __eq__(self, __o: object) -> bool:
        if isinstance(__o, ConfigKey):
            return self.key == __o.key
        elif isinstance(__o, str):
            return self.key == __o
        return False

    def __add__(self, __o: str | "ConfigKey") -> "ConfigKey":
        if isinstance(__o, ConfigKey):
    ...
    def __radd__(self, __o: str | "ConfigKey") -> "ConfigKey":
        if isinstance(__o, ConfigKey):
...
class BaseField(Generic[T_Value]):
    ...
    def setKey(self, key: str | ConfigKey) -> None:
        if isinstance(key, ConfigKey):
            self.key = key
        elif isinstance(key, str):
            self.key = ConfigKey(key)

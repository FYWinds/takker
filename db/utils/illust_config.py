...
class IllustConfig:
    ...
    async def get_illust_config() -> dict[str, bool]:
        ...
        if config and config.illust_config:
            ...
            await IllustConfig._init_illust_config()
            return await IllustConfig.get_illust_config()

    @staticmethod
    async def _init_illust_config() -> None:
        await BotConfig.update_or_create(
            id=1, defaults={"illust_config": default_config}
        )
    ...
    async def set_illust_config(config: Union[str, list[str]], status: bool) -> None:
        ...
        await BotConfig.update_or_create(
            id=1, defaults={"illust_config": current_config}
        )

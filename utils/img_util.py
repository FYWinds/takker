# 从真寻那抄来的img_utils，加了很多  # type: ignore
import base64
...
def alpha2white_pil(pic: Image.Image) -> Image.Image:
    """
    说明：
        将图片透明背景转化为白色
    参数：
...
class CreateImg:
    ...
    def __init__(
        ...
        self.current_w = 0
        self.current_h = 0
        self.font = ImageFont.truetype(FONT_PATH + ttf, int(font_size))
        if not background:
            if plain_text:
                ttf_w, ttf_h = self.getsize(plain_text)
                self.w = self.w if self.w > ttf_w else ttf_w
                self.h = self.h if self.h > ttf_h else ttf_h
            self.markImg = Image.new(image_mode, (self.w, self.h), color)  # type: ignore
    ...
    def paste(
        self,
        img: "CreateImg" or Image,  # type: ignore
        ...
        if alpha:
            try:
                self.markImg.paste(img, pos, img)  # type: ignore
            except ValueError:
                img = img.convert("RGBA")  # type: ignore
                self.markImg.paste(img, pos, img)  # type: ignore
        else:
            self.markImg.paste(img, pos)  # type: ignore
    ...
    def show(self):
        ...
        说明：
            显示图片
        """
        self.markImg.show(self.markImg)  # type: ignore

...
class Dynamic:
    ...
    async def format(self, img):
        ...
        if self.type == 8:
            video_img = await self.create_img()
        self.message = (
            (
                f"{self.name} "
            ...
            if self.type != 8
                ...
                + MessageSegment.image(f"base64://{img}")
                + "\n"
                + MessageSegment.image(video_img)  # type: ignore
                + f"\n{self.url}"
            )
        )
    ...
    async def create_img(self):
        ...
        def binfo_image_create(video_info):
            bg_y = 0
            # 封面
            ...
            dynamic_font = ImageFont.truetype(
                f"{FONT_PATH}sarasa-mono-sc-semibold.ttf", 18
            )
            ...
            video.paste(up_bg, (20, 390 + title_bg_y + 20 + dynamic_bg_y + 10))
            video.paste(
                baner_bg, (0, 390 + title_bg_y + 20 + dynamic_bg_y + 10 + up_bg_y)
            )
        ...
        video_info = await video_info_get(self.bvid)
        img = binfo_image_create(video_info)
        return img

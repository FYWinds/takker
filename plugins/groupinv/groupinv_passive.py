...
async def check_list(user_id: str):
    ...
    g2 = await Info.get_group_member_list(319152433)
    g3 = await Info.get_group_member_list(603809278)
    g4 = await Info.get_group_member_list(869202661)
...
async def check_level(user_id: int):
    r = await Info.get_stranger_info(user_id)
    if r["level"] >= 15:
        return True
    else:
        return False
